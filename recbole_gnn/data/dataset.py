...
class GeneralGraphDataset(RecBoleDataset):
    ...
    def get_norm_adj_mat(self):
        ...
        row = self.inter_feat[self.uid_field]    # 矩阵的行是uid
        col = self.inter_feat[self.iid_field] + self.user_num   # 矩阵的列是iid，注意因为要把uid和iid放在同一表示空间，所以iid列表拼接到了uid列表之后（iid + num_user）
        edge_index1 = torch.stack([row, col])   # row->col  边索引
        edge_index2 = torch.stack([col, row])   # col->row  边索引（逆向）
        edge_index = torch.cat([edge_index1, edge_index2], dim=1)   # 最终边索引（正向+反向）

        deg = degree(edge_index[0], self.user_num + self.item_num)  # 计算所有节点的度

        norm_deg = 1. / torch.sqrt(torch.where(deg == 0, torch.ones([1]), deg))  # 标准化（D^{-0.5}）
        edge_weight = norm_deg[edge_index[0]] * norm_deg[edge_index[1]]     # 计算所有边的权重值（聚合时用的权重）
